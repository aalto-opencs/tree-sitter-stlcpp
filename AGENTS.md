...
- `src/grammar.json` — generated by `tree-sitter generate`. Represents the compiled grammar.
...
- `../stlcpp/examples/` — (upstream examples) used for testing examples parsing. Relative path usage is common in previous workflow.
...
- Edit only `grammar.js` and any support files (`queries/`) by hand.
- After `tree-sitter generate`, include the generated artifacts (`src/grammar.json`, `src/node-types.json`, `src/parser.c`) in the same commit as your `grammar.js` change so CI/editors using the parser see a consistent state.
- Keep commit messages focused: explain the grammar change and include a short rationale (e.g., "Make term_primary the canonical atom; prevent parentheses from hiding full expr" or "Allow newline after '=' in value definitions").
...
- Always run `tree-sitter generate` after editing `grammar.js`. If you don't have the ability to run the generator in the environment, do not commit `src/*` changes — instead open a PR with `grammar.js` and a note that the generated artifacts need regenerating.
- If you cannot actually run `tree-sitter parse`, you can still reason about grammar changes conservatively, but clearly indicate that your changes are untested and request that a human run the generator and parser locally.
